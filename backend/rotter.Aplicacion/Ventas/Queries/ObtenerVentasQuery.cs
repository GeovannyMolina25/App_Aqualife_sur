...
public record ObtenerVentasQuery(int Pagina = 1, int Tamano = 20, DateTime? Desde = null, DateTime? Hasta = null, string? Busqueda = null)
...
public class ObtenerVentasHandler : IRequestHandler<ObtenerVentasQuery, RespuestaDto<PagedResult<VentaDto>>>
{
    ...
    {
        var lista   = await _ventas.ObtenerTodasAsync(req.Pagina, req.Tamano, req.Desde, req.Hasta, req.Busqueda);
        var total   = await _ventas.TotalAsync(req.Desde, req.Hasta, req.Busqueda);
        var dtos    = lista.Select(RegistrarVentaHandler.Mapear).ToList();
        var paginas = (int)Math.Ceiling(total / (double)req.Tamano);
        return RespuestaDto<PagedResult<VentaDto>>.Ok(new(dtos, total, req.Pagina, req.Tamano, paginas));
    }
}
